...
import com.flino.config.dto.toDomain
import com.flino.config.exception.DuplicateConfigNameException
import com.flino.config.filestore.ConfigId
...
class ConfigService(
    ...
    /** Used by [com.flino.config.seed.SyncSpecConfigSeeder] to skip a SyncSource spec that's already been imported (or has since been user-edited under the same name). */
    fun existsByName(name: String): Boolean = store.isNameTaken(name, excludingId = null)
