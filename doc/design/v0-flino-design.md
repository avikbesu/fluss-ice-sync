...
Build a configurable Java application, **flino**, that watches configured
...
a sync client such as Dropbox, or a person manually placing a file); flino
...
flino removes that manual step: once a source is configured, any file
...
* Coordinating multiple flino replicas processing the same source
...
flino/
├── app/
│   └── sync/                    # flino application module (Java)
│       ├── src/main/java/...
│       ├── src/test/java/...
│       └── Dockerfile
├── config/
...
│   │   ├── docker-compose.app.yml         # flino service
...
* `config/docker` and `config/resources` are deliberately separate from
  ...
  bind-mounted together into the `flino` container (see
...
* `spec.security.roles` names the role(s) required to write to the
  destination table, enforced wherever flino's own service identity is
...
create event does not mean the file is complete. flino uses a **quiet
...
flino moves files into them — re-triggering detection on files
flino itself just placed there. To prevent this, the watcher excludes
...
* On startup, and again lazily on first use of a source, flino uses the
...
flino maintains an embedded, on-disk ledger (a lightweight local key
store, e.g. SQLite or MapDB — file: `state/ledger.db`) keyed by
`(sourceName, filePath, contentHash)` → `{status, processedAt}`.

* Before processing a file, flino checks the ledger. If an entry exists
...
flino exposes an HTTP health endpoint (`config/apps/sync/application.yaml`'s
...
v0 targets **Docker Compose** as the deployment mechanism, running flino
...
flino and Fluss be brought up together with one command, matching the
...
* `docker-compose.app.yml` — the `flino` service.
...
* `docker-compose.app.yml`'s `flino.depends_on: [tablet-server]` only
...
* `/config/resources` and `/config/application.yaml` are mounted read-only
  ...
  `apps/sync/` nesting.
  Config changes require restarting the `flino` container (consistent
...
* The `flino` image is built from `app/sync/Dockerfile` — kept next to
...
| `make run` | `./gradlew :app:sync:run` — run flino locally against an already-running Fluss cluster |
...
| `make up` | `docker compose -f config/docker/docker-compose.infra.yml -f config/docker/docker-compose.app.yml -f config/docker/docker-compose.monitoring.yml up -d --build` — bring up the full stack, always rebuilding the `flino` image from local source first |
| `make down` | same `-f` list as `make up`, with `down` |
| `make logs` | same `-f` list as `make up`, with `logs -f flino` |
...
local, already-synced folder, and it would tie flino's core watch logic
...
* What is the retention/cleanup policy for the `_processed` archive
  subfolders and the `_rejected` folder — is that managed by flino or
...
* **Fluss cluster authentication.** How flino itself authenticates to
